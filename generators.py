...
def generators_inputs(gen_params, t):
    ...
    sup_pyr_mean = np.zeros(t.size, dtype=np.float32)
    deep_pyr_mean = np.zeros(t.size, dtype=np.float32)

    for i in range(t.size):

        mec_mean[i] = get_gen_mean(t[i], gen_params['mec'])
        lec_mean[i] = get_gen_mean(t[i], gen_params['ca3'])
        if len(gen_params) == 4:
            sup_pyr_mean[i] = get_gen_mean(t[i], gen_params['sup'])
            deep_pyr_mean[i] = get_gen_mean(t[i], gen_params['deep'])
    ...
    return mec_mean, lec_mean, sup_pyr_mean, deep_pyr_mean
